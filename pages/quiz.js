...
<div class="shopping-list">${al.map((a,i)=>{const fd=FUND_DETAILS[a.code];const etfHint=fd&&fd.etfCode?` · 场内ETF:${fd.etfCode}`:'';const aiC=_recAiComments[a.code];const aiR=a.ruleReason||a.aiReason;const reasonIcon=a.ruleReason?'⚙️':'🤖';const isStock=a.assetType==='stock'||(a.code&&/^(sh|sz)\d{6}$/.test(a.code));const typeTag=isStock?'<span style="display:inline-block;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600;background:rgba(239,68,68,.15);color:#EF4444;margin-left:6px">📊 个股</span>':a.code==='余额宝'?'<span style="display:inline-block;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600;background:rgba(16,185,129,.15);color:#10B981;margin-left:6px">💵 货基</span>':'<span style="display:inline-block;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600;background:rgba(59,130,246,.15);color:#3B82F6;margin-left:6px">🏦 基金</span>';const platformText=a.code==='余额宝'?'留在余额宝/零钱通':isStock?'⚠️ 仅券商APP可买（如东方财富/同花顺/雪球）':'支付宝·天天基金·券商均可买';return`<div class="shop-item" onclick="showFundDetail('${a.code}')"><div class="shop-num">${i+1}</div><div class="shop-detail"><div class="shop-fund-name">${a.fullName||a.name}${typeTag}</div><span class="shop-code">${a.code}${etfHint}</span><span class="shop-platform"${isStock?' style="color:#EF4444"':''}>${platformText}</span>${aiR&&!aiC?`<div style="margin-top:6px;padding:6px 10px;background:rgba(59,130,246,.08);border-radius:8px;font-size:12px;color:#60A5FA;line-height:1.5">${reasonIcon} ${aiR}</div>`:''}${aiC?`<div style="margin-top:6px;padding:6px 10px;background:rgba(139,92,246,.08);border-radius:8px;font-size:12px;color:#A78BFA;line-height:1.5">🤖 ${aiC}</div>`:''}${liveNavData[a.code]?`<div class="shop-live-nav">📡 净值:${liveNavData[a.code].nav}(${liveNavData[a.code].date})</div>`:''}</div><div class="shop-amount">${fmtFull(Math.round(amt*a.pct/100))}</div></div>`}).join('')}</div>
