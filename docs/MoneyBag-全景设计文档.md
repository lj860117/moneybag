...
| **V8.3.3** | **自动调权引擎（13因子+推荐+regime）** | **2 天** | **灰度部署+影子模式+紧急回滚通过** |
...
## V8.3.3 自动调权引擎（13 因子信号 + 推荐引擎权重）

> **问题**：V8.3 雏形里"错得多就降权 15%"过于粗糙——
> - 固定阈值（>25% 触发）+ 固定步长（×0.85），**没有学习率**
> - **调完不验证**：调后下周表现更差也不知道，无法回滚
> - **没有约束**：可能调成单一因子 80%、其他全部 0%
> - 只调 `RECOMMEND_WEIGHTS`（5 维），没碰 **`SIGNAL_WEIGHTS_V5`（13 维）** 和 **`REGIME_WEIGHTS`（4 态）**
> - 全市场一套权重，没有 **per-user / per-regime** 分层
>
> **方案**：把"自动调权"工程化为独立子系统，含数据准备、调权算法、约束检查、灰度回滚、性能验证 5 个环节。

### 一、调权对象（3 套权重表）

| 权重表 | 来源 | 维度 | 调权频率 | 风险 |
|--------|------|------|---------|------|
| `SIGNAL_WEIGHTS_V5` | `config.py` | 13 维（技术/资金/估值/情绪/宏观/盘面/地缘/...） | **月度** | 高 — 影响整个 regime 判定 |
| `RECOMMEND_WEIGHTS` | `config.py` | 5 维（估值30/盈利25/技术15/资金15/风险15）+ 三周期变体 | **周度** | 中 — 影响推荐排序 |
| `REGIME_WEIGHTS` | `regime_engine.py` | 4 态（risk_on/cautious/risk_off/neutral）映射 | **季度** | 极高 — 影响整个 pipeline 选择 |

> 🔴 铁律：`SIGNAL_WEIGHTS_V5` 和 `REGIME_WEIGHTS` 改动必须 **B 级人工确认**，不允许 A 级自动改。

### 二、自动调权数据基础（依赖 V8.1 / V8.2）

```python
# data/weight_tuning/dataset.json — 调权用数据集
{
  "decisions": [          # 来自 V8.1 决策日志
    {"date": "2026-04-15", "ticker": "600519",
     "predicted": "buy", "actual_return_5d": -0.03,
     "factor_snapshot": {"valuation": 0.7, "earnings": 0.6, ...},
     "signal_snapshot": {"tech": 60, "fund": 45, ...}}
  ],
  "attribution_history": [...],  # 来自 V8.2，每周复盘的归因结果
  "regime_history": [...]        # 当时市场状态
}
```
...
```python
# services/weight_tuner.py — V8.3.3 新增

class WeightTuner:
    """三种算法，按数据量和稳定性渐进升级"""

    # 算法 A：归因驱动（V8.3.3 first ship）
    # 数据需求：≥4 周复盘 → 最低门槛
    def tune_by_attribution(self, attribution_history, current_weights):
        """
        基于 V8.2 归因结果调权，自带学习率 + 约束。
        - 错误率 e_i：维度 i 在归因中占比
        - 学习率 lr：根据数据置信度，4周=0.05 / 12周=0.10 / 24周+=0.15
        - 调权公式：w_new = w_old * (1 - lr * (e_i - mean_e))
        """
        avg_error = sum(attribution_history[-1].values()) / len(current_weights)
        lr = self._adaptive_lr(len(attribution_history))
        new_weights = {}
        for k, w in current_weights.items():
            err = attribution_history[-1].get(self._map(k), avg_error)
            new_weights[k] = w * (1 - lr * (err - avg_error))
        return self._apply_constraints(new_weights)

    # 算法 B：grid search（V8.4+，需积累 12 周决策日志）
    def tune_by_grid_search(self, dataset, base_weights):
        """对每个权重在 ±20% 范围内 5 步搜索，回测最优组合"""
        ...

    # 算法 C：贝叶斯优化（V9+，需积累 24 周）
    def tune_by_bayesian(self, dataset, base_weights):
        """用 scikit-optimize，目标函数 = 准确率 - 0.5 * 回撤"""
        ...
    ...
    def _apply_constraints(self, weights):
        ...
        for k in weights:
            weights[k] = max(0.02, min(0.40, weights[k]))   # 单维 [2%, 40%]
        # 类别下限：技术面+资金面合计 ≥ 25%
        tech_fund = sum(weights.get(k, 0) for k in ['tech', 'fund', 'volume'])
        if tech_fund < 0.25:
            scale = 0.25 / tech_fund
            for k in ['tech', 'fund', 'volume']:
                if k in weights:
                    weights[k] *= scale
        return self._normalize(weights)

    def _adaptive_lr(self, weeks):
        if weeks < 4:  return 0.0      # 不调
        if weeks < 12: return 0.05
        if weeks < 24: return 0.10
        return 0.15
```
...
```python
# services/weight_tuner.py 续

class WeightDeployment:
    """每次调权进入"灰度池"，2周观察期，A/B 对比"""

    def deploy_gray(self, new_weights, current_weights):
        """
        新权重不直接替换，而是同时运行：
        - 主权重（current）：实际产生推荐 → 用户看到
        - 灰度权重（new）：影子模式，记录推荐但不展示
        - 2 周后对比两套的 5 日命中率、夏普比
        """
        gray_record = {
            ...
            "shadow_results": [],
            "status": "observing",  # observing → promoted / rolled_back
            "evaluation_at": (datetime.now() + timedelta(days=14)).isoformat()
        }
        save_json(f'data/weight_tuning/gray_{run_id}.json', gray_record)

    def evaluate_gray(self, gray_id):
        """
        2 周后自动评估：
        - 新权重命中率提升 ≥ 3% → promoted（写入 config）
        - 提升 < 3% 或下降 → rolled_back（保留日志）
        - 命中率下降 > 5% → 立即回滚 + 企微告警
        """
        ...
...
### 五、per-user / per-regime 分层（V8.3.3 second ship）

```python
# 用户分层
RECOMMEND_WEIGHTS_BY_USER = {
    'u_1ce33b': {...},   # LeiJiang growth 个性化权重
    'u_dc4d4068': {...}, # BuLuoGeLi balanced 个性化权重
}
# 启用条件：用户决策日志 ≥ 30 条

# 市场状态分层（regime-aware）
SIGNAL_WEIGHTS_BY_REGIME = {
    'risk_on':   {'tech': 0.20, 'fund': 0.18, ...},  # 牛市重技术
    'cautious':  {'valuation': 0.18, 'risk': 0.15},  # 震荡重估值
    'risk_off':  {'risk': 0.20, 'geopol': 0.15},    # 熊市重风险
    'neutral':   SIGNAL_WEIGHTS_V5,                   # 默认
}
```
...
| # | 任务 | 改动文件 | 工时 | 优先级 |
|---|------|----------|------|-------|
| V8.3.3a | WeightTuner 类 + 算法 A（归因驱动） | `services/weight_tuner.py`（新建） | 3h | P0 |
| V8.3.3b | 约束检查 + 归一化 | `services/weight_tuner.py` | 1h | P0 |
| V8.3.3c | 灰度部署 + 影子模式 | `services/weight_tuner.py` | 2h | P0 |
| V8.3.3d | 紧急回滚 + 企微告警 | `services/weight_tuner.py` + `services/wechat_push.py` | 1h | P0 |
| V8.3.3e | 接入 V8.3 复盘流程（A 级自动 / B 级确认） | `services/review_engine.py` | 1h | P0 |
| V8.3.3f | 调权历史 API + Pro 前端 | `main.py` + `app.js` | 1.5h | P1 |
| V8.3.3g | per-user 个性化权重 | `services/weight_tuner.py` + `config.py` | 2h | P1 |
| V8.3.3h | per-regime 分层权重 | `regime_engine.py` + `signal.py` | 2h | P2 |
| V8.3.3i | 算法 B（grid search） | `services/weight_tuner.py` | 2h | V8.4 |
| V8.3.3j | 算法 C（贝叶斯优化） | `services/weight_tuner.py` | 4h | V9 |

**P0 合计 8h，P1 合计 5.5h，P2 合计 2h → V8.3.3 ≈ 2 天（含自测）**

### 七、自测验证

| # | 验证项 | 方法 | 通过标准 |
|---|--------|------|---------|
| ① | 算法收敛 | 构造 12 周归因数据 → 跑 `tune_by_attribution` | 权重变化 ≤ 15%，归一化后 sum=1.0 |
| ② | 约束生效 | 构造极端归因（某维错 80%） → 调权 | 该维不会 < 2%，其他维不会 > 40% |
| ③ | 类别下限 | 构造技术+资金归因都很差的案例 | 调权后 tech+fund+volume ≥ 25% |
| ④ | 灰度部署 | 调用 `deploy_gray(new, old)` | `data/weight_tuning/gray_*.json` 写入 |
| ⑤ | 影子记录 | 灰度期间跑推荐 | gray_record.shadow_results 累积 |
| ⑥ | 自动晋升 | 模拟新权重 +5% 命中率 → `evaluate_gray` | status: promoted，config.py 更新 |
| ⑦ | 自动回滚 | 模拟新权重 -8% 命中率 | status: rolled_back，企微告警 |
| ⑧ | 紧急回滚 | 模拟连续 3 天命中率 < 30% | 灰度立即停用，企微红色告警 |
| ⑨ | per-user | 用户决策 < 30 条 → 用全局权重；≥ 30 → 用个性化 | 切换正确 |
| ⑩ | per-regime | 4 种市场状态各跑一次推荐 | 用对应权重表 |
| ⑪ | A/B 自动 | 归因 weight_adj 全部在 ±15% 内 | A 级，自动改 config |
| ⑫ | B/C 确认 | 调权超过 ±15% 或动 SIGNAL_WEIGHTS_V5 | B 级，推企微等用户确认 |
| ⑬ | 历史可查 | `curl /api/weight-tuning/history?weeks=12` | 返回每次调权的 before/after/result |
| ⑭ | Pro 前端 | Pro 模式 → 复盘页"权重调整历史" Tab | 折线图：每个权重随时间变化 |
...
|------|------|
| **铁律 #3** | 每项做完跑自测 |
| **铁律 #18** | `/api/weight-tuning/*` 后端做了 → 前端 Pro 复盘页展示 |
| **铁律 #19** | 多用户调权用 `userId` 入口，不允许全局污染 |
| **铁律 #E.2** | 调权失败/数据不足时降级到固定权重，不报错阻塞 |
| **代码组织** | 新逻辑独立在 `weight_tuner.py`，复盘引擎只调用不实现 |
| **依赖** | V8.1（决策日志）+ V8.2（归因）→ 必须先完成 |
| **数据积累** | 算法 A 至少 4 周 / B 至少 12 周 / C 至少 24 周；不满足直接跳过调权 |

### 九、与 V6.5/V7 的衔接

| V6.5/V7 现状 | V8.3.3 改造 |
|--------------|------------|
| `SIGNAL_WEIGHTS_V5` 硬编码在 `config.py` | 改为可热更新（保留 `SIGNAL_WEIGHTS_V5_DEFAULT` 兜底） |
| `RECOMMEND_WEIGHTS` 三周期版本（short/medium/long）硬编码 | 各周期独立调权，per-user 时再分一层 |
| `REGIME_WEIGHTS` 在 regime_engine.py 写死 | 调权时只动数值，不动 4 态结构 |
| 推荐引擎按风险类型调权（growth/balanced/conservative） | V8.3.3 在此基础上叠加 per-user 学习层 |

### 十、设计要点总结

> 🎯 **V8.3.3 的核心价值不是"会调权"，而是"调错了能发现、能回滚、不毁掉系统"**
>
> 1. **学习率自适应**：数据少 → 不调；数据多 → 大胆调
> 2. **硬约束**：单维 [2%, 40%]，类别下限 25%
> 3. **影子模式**：新权重 2 周观察期，不影响真实推荐
> 4. **三级回滚**：定期评估 → 紧急回滚 → 用户反馈兜底
> 5. **分级执行**：A 级自动（小调）/ B 级确认（大调或动 13 维）
> 6. **per-user 学习**：用户决策够了再开个性化层
> 7. **versioned config**：每次调权产生 `config_v8_3_3_run20260418.json`，可任意回滚到历史版本
...
| **V8.3.3** | **自动调权引擎（13因子+推荐+regime+灰度+回滚）** | **2天** |
| V8.4 | 触发机制 | 1天 |
| V8.5 | 前端+推送 | 1.5天 |
...
- [ ] **V8.3.3** 自动调权算法收敛 + 约束生效 + 灰度部署 + 紧急回滚通过
- [ ] **V8.3.3** 调权历史 API + Pro 复盘页"权重调整历史" Tab 正常展示
