...
> #### 📋 Phase 0 完成状态（2026-04-17 实际部署结果）
>
> **后端 ✅ 全部完成**（13 个 API 验证通过，已部署）：
> - [x] 数据源巡检 / 覆盖率检查 / 原子写 / 缓存持久化 / 操作日志 / 决策日志
> - [x] 用户偏好 API / 家庭汇总 API / 盯盘预警 API / Token 预算 / 意图分类
> - [x] uvicorn ×1 / 版本号 6.0.0-phase0
>
> **前端 ⚠️ 部分完成**（旧版 app.js + 5 个安全补丁）：
> - [x] 主题切换（亮/暗/系统）
> - [x] 模式切换同步后端偏好
> - [x] 启动偏好恢复
> - [x] renderCard 三态组件
> - [x] 盯盘轮询 + visibilitychange
> - [ ] ❗ **空仓首页改造**（后端 household API 已有，前端模板未插入）
> - [ ] ❗ **资讯页新增 Tab**：deepimpact / riskassess（后端 API 已有）
> - [ ] ❗ **持仓页 AI 分析按钮**（后端 analyze API 已有）
> - [ ] ❗ **信号页 AI 解读**（后端 interpret API 已有）
> - [ ] ❗ **首页入场时机卡片**（后端 timing/smart-dca API 已有）
>
> **原因**：旧版 app.js 是压缩单行格式，replace 插入大段 HTML 模板极易截断函数体导致白屏。
> **建议**：V6 前端改动时，先把 app.js 格式化（prettier）或拆成模块，再做上述 6 项前端接入。
