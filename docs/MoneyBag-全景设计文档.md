...
> 📅 最后更新：2026-04-16 02:49  
> 📝 包含：Phase 0 + V6 + V6.5 + V7 + V8 + V9 + V10+ 全部设计（5024 行）  
...
02:00  🧠 R1 Phase 1：全局市场                  [R1×3] ¥0.18
       ├── ① 宏观环境研判（综合判断，需要推理）     [R1]
       ├── ② 地缘政治影响（复杂传导链）             [R1]
       └── ③ 行业轮动分析（跨行业比较）             [R1]
...
> 腾讯云 2C2G，uvicorn 2 worker ~600MB，凌晨任务需控制在 400MB 以内
...
总内存：2048 MB
├── 系统 + OS：~300 MB
├── uvicorn ×2 worker：~600 MB（常驻）
├── night_worker：~400 MB（01:00-08:00，用完释放）
├── 安全余量：~200 MB（触发保护的阈值）
└── 峰值：~1500 MB（凌晨任务执行时）

白天（无凌晨任务）：~900 MB 使用 / 1100 MB 空闲 ✅
凌晨（任务执行中）：~1300 MB 使用 / 700 MB 空闲 ✅
最差（R1 返回大文本）：~1500 MB 使用 / 500 MB 空闲 ⚠️ 够用
```
