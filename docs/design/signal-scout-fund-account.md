...
| 状态 | **只读设计**，不修改任何现有代码 |
| 🔴 **数据状态（2026-09-04 追加）** | 上游 PRD 的持仓数据是**等权测试值**（7 只各 100 元 + 1 只 10 元，8 条 `addedAt` 全同），**非用户真实持仓**，用户将重录。**本设计里所有标注 🧪 的数字均为等权测试值下的示例，重录后作废**，不得固化为回归断言或硬编码常量。详见 PRD §0.0 与下方 §0 |
...
## 0. 🧪 等权测试数据对本设计的影响范围（2026-09-04 追加）

**事实**：`/opt/moneybag/data/fund_holdings_LeiJiang.json` = 8 只基金，**7 只各 100 元 + 007356 仅 10 元**，总成本 709 元，8 条 `addedAt` **完全相同**。这是批量造的测试数据，用户（LeiJiang）将重录真实持仓。

**设计层面的结论（重要）**：本设计的**架构、模块划分、状态机、B8/B1/B6 修复、覆盖率运行时判定**全部**不受影响** —— 它们描述的是「怎么算」，不是「算出来是多少」。受影响的只有**被写进设计文档的示例数字**，以及**把这些示例数字固化成断言的验收标准**。

| 本设计中的内容 | 是否受等权测试数据影响 | 处理 |
|---|---|---|
| 模块划分 / 文件清单 / 接缝设计 / 状态机 / 预算守门 | ❌ 不受影响 | 照原设计实现 |
| `weight_mv = market_value / Σ market_value` 这个**公式** | ❌ 不受影响 | 照原设计实现；输入换成真实持仓后自动算对 |
| 覆盖率三分解 `penetrated + blind + residual == 100` 这个**恒等式** | ❌ 不受影响 | 保留为断言（恒等式永远成立） |
| 🧪 三分解的**具体数值** 61.8 / 24.4 / 13.8（线上 63.0 / 25.1 / 12.0） | 🔴 **作废** | **降级为注释，禁止写成 `≈` 断言**，见 §5 T03 |
| 🧪 半导体 33.5%、北方华创 4.53% 等**穿透暴露示例值** | 🔴 **作废** | 同上；`fund_count == 4` 这类**计数**结论不受影响 |
| 🧪 文案模板里所有「占你总净值 X%」的占位数字 | 🔴 **作废** | 句式照抄、数字运行时算，见 §4.3 |
| 单只基金百分比（-26.9%、+18.8%、+11.1%）与 B8 的 `unit_nav` 规则 | ✅ 不受影响 | 照原设计实现；002163 的 +161.37% 陷阱与权重无关 |
| 🧪 测试夹具 `tests/fixtures/fund_holdings_leijiang.json` | 🔴 **是第三套编造数据** | 见 §7，与生产数据的 `costNav` **有 3 只对不上** |

> ### 对实现者的一句话
> **照着公式写，不要照着数字写。** 本设计文档里每一个具体百分比都只是「当时跑出来的样子」，不是规格。
...
| D7 | QDII 🧪24.43%（线上 🧪25.1%）永久无穿透 | 覆盖率**运行时算**，禁止硬编码；不足时文案强制点名盲区清单。<br>🧪 占比数字会随真实持仓变化，**「运行时算、禁止硬编码」这条对策因此从「好习惯」升级为正确性底线** —— 若硬编码 61.8%，重录真实持仓后系统会持续推送错误覆盖率，而用户几乎不可能察觉 |
...
def compute_exposure(positions: list[FundPosition], portfolios: dict[str, dict],
    ...
    覆盖率三分解（三者相加恒 = 100%）：
        penetrated_pct = Σ_stocks exposure_pct                     # 🧪61.8%（线上 🧪63.0%）
        blind_pct      = Σ weight_mv of funds where ok=False       # 🧪24.4%（QDII，线上 🧪25.1%）
        residual_pct   = Σ weight_mv of ok funds - penetrated_pct  # 🧪13.8%（前十大之外+现金债券，线上 🧪12.0%）
      ⚠️🧪 上面三个数值是【等权测试持仓】下跑出来的样子（7×100元 + 1×10元），
      用户重录真实持仓后全部会变。**只能当作格式示意，禁止写进回归断言 `≈`**，
      断言只需保留恒等式 penetrated + blind + residual == 100.00。
      验证：016501 权重 🧪15.56% × 北方华创 9.89% = 🧪1.54%（单基金）；
            北方华创被 4 只基金重仓 → 合计 🧪4.53%（= 占用户总净值，线上 🧪4.48%）
        ↑ 其中 9.89%（基金自身披露的持仓比例）与「被 4 只基金重仓」（计数）不受影响，
          受影响的是 15.56% 这个权重与 1.54%/4.53% 这两个加权结果。

    触发规则（命中任一即 should_push=True）：
        R1 单一申万二级行业 exposure_pct > 25.0     # 🧪 受等权测试数据影响，可能翻转
        R2 单一个股 exposure_pct > 3.0             # 🧪 受等权测试数据影响，可能翻转
        R3 单一个股 len(via_funds) >= 3            # ✅ 纯计数，不受影响
        R4 冷启动首次启用（无条件 1 次基线体检）     # ✅ 不受影响
      阈值 25.0 / 3.0 / 3 本身是产品决策，不随持仓变化。"""
...
> 🧪 **下方 4 个模板里所有「占你总净值 X%」「穿透覆盖 61.8%」「半导体 33.5%」「北方华创 4.53%」等数字，都是等权测试持仓下的示例输出，不是用户会看到的真实内容。**
>
> **句式照抄，数字全部运行时计算。** 分类如下：
>
> | 模板 | 受影响程度 | 说明 |
> |---|---|---|
> | 🧪 `P0-1 xray` | **最大** | 主体就是集中度与穿透暴露，几乎每个数字都是权重类 |
> | 🧪 `P1-1 dca` | **大** | 「组合整体 -2.2%」依赖持仓份额（口径 A，见下）；集中度段是权重类 |
> | 🧪 `P0-3 drawdown` | **小** | 主体是单只基金的 `unit_nav/cost_nav`，可用；仅「占你总净值 Z.Z%」与「其余 7 只（占 89.5%）」是权重类 |
> | 🧪 `P0-2 manager` | **小** | 主体是变更事件；仅「占你总净值 12.5%」是权重类 |
>
> ⚠️ 特别提醒：`P0-3` 模板里的 **-26.9%、+18.8%、+11.1% 与「3 只浮盈」不受影响**（单只基金百分比，只依赖 `unit_nav/cost_nav`），但**模板下方的 §4.3 尾注把它们与组合口径混在同一段里说明**，读的时候要分开看。
...
组合整体：相对成本 -2.2%（8 只基金按持仓成本加权）
回撤最深：华夏全球科技先锋QDII-A(005698) -26.9%，占你总净值 10.5%
表现最好：华夏先进制造龙头混合A(013107) +18.8%，占你总净值 17.1%

集中度提醒（持仓截止 2026-06-30，滞后 21 天，穿透覆盖 61.8% 净值）：半导体 33.5% ｜ 通信设备 13.3%
本期定投若仍投向半导体，集中度将进一步上升
...
> 🔴 **三处与 `render.py` 对齐的修正（2026-09-05，工程师代码已落地，文档同步）**
>
> **① 结论行的行业名必须来自穿透结果，禁止硬编码**（`render.py:259-264`）
>
> 原模板写「本期定投若仍投向**科技成长**」，与同模板上一行算出的第一大行业「**半导体**」**自相矛盾** —— 同一个模板里两个行业名打架。且当前是等权测试数据，重录真实持仓后第一大行业可能整个换掉，写死就是在数据失效后继续输出明确错误的陈述。
>
> ```
> 第一大行业 = xray.industries[0].industry     # 已按 exposure_pct 降序，见 xray.compute_exposure
> if xray is not None and xray.industries:
>     lines.append(f"本期定投若仍投向{xray.industries[0].industry}，集中度将进一步上升")
> # ⚠️ 拿不到穿透数据 → 整行不输出，禁止 fallback 任何硬编码行业词
> ```
> **宁可少一行，不可错一行。** 模板已改为 `本期定投若仍投向{第一大行业}`，与上一行的行业名保持一致。PRD §6.3 P0-4 模板同步修正（该处是硬编码词的源头）。
>
> **② 「相对成本」不是「相对成本净值」**（`render.py:236`）
>
> 原文案 `相对成本净值 -2.2%` 会让人以为 −2.2% 是一个净值数值，实际它是**百分比**。代码已改为：
> ```
> f"组合整体：相对成本 {combo_s}（{total} 只基金按持仓成本加权）"
> ```
> 括号标注同步为**「按持仓成本加权」**（口径 A = `Σmv/Σcost−1`）。⚠️ 这个标注必须跟着 `combo_cost_pct` 走：写「按市值加权」会把口径 A 说成口径 B，而 `B ≥ A` 恒成立 —— 用户会以为自己亏得比实际少。**改数字不改这里就是错的。**
>
> **③ 「集中度提醒」行：补上持仓截止日 + 滞后天数**（`render.py:250-265`）
>
> PRD §6.1 强制规则 4（必须标注数据时点与滞后天数）+ §6.1.1 H1（**明确把「P0-4 定投提醒的集中度段」列为穿透类内容**）要求这一行自带时点。数据一直都在 `xray.py` L49-51 的 `Coverage` dataclass（`end_date` / `ann_date` / `lag_days`，注释原话「供文案标注滞后」），只是 `render.py` 原先没用，现已补上。
>
> **最终形态（整行逐字相等，已锁测试）**：
> ```
> 集中度提醒（持仓截止 2026-06-30，滞后 21 天，穿透覆盖 61.8% 净值）：半导体 33.5% ｜ 通信设备 13.3%
...
> | # | 行为 | 触发条件 | 结果 |
> |---|---|---|---|
> | **1 守卫** | `if xray is not None and xray.industries:` | `industries` 为空（组合全是 QDII / 申万行业表两级降级都失败，见 `sw_industry.py`） | **整行不输出** —— 与结论行一致：拿到数据才说话。避免了「冒号 + 空白」的空壳行 |
> | **2 降级** | `end_date` 为空 或 `lag_days <= 0` | 时点算不出来 | `stamp = ""`，**只丢「持仓截止 X，滞后 N 天，」子句**，覆盖率和行业敞口本身依然成立，整行照常输出：<br>`集中度提醒（穿透覆盖 61.8% 净值）：半导体 33.5% ｜ 通信设备 13.3%` |
> | **3 前提** | 1 通过后才谈 2 | — | **守卫优先于降级**：没行业敞口时，哪怕时点算得出来也整行不输出 |
>
> 🚫 **两个坑（写在这里防后人踩，都是本项目真实踩过的）**：
>
> 1. **不要用 `_fmt_md` 压这个日期。** 它会把 `2026-06-30` 砍成 `06-30`，**丢掉年份**。持仓截止日来自季报（可能半年前），显示成「06-30」用户会误以为是最近几天的数据，也不满足 §6.1 规则 4 要求的 `YYYY-MM-DD`。
>    > 📌 **这条值得一提**：前一版规格里**我自己写错了**，让工程师用 `_fmt_md` 压日期，后来自己发现更正。**教训：凡是季报/年报这类可能跨年的滞后数据，一律保留完整 `YYYY-MM-DD`，不要为了省字符砍年份。**
> 2. **不要用顶部的 `snap["nav_date"]` 顶替。** 那是**净值日**（QDII 滞后 2 天），而穿透数据来自**季报**（滞后 21~28 天）。两者混在一起正是 §6.1 规则 4 要防的口径污染 —— 用户会以为自己看到的是 2 天前的数据，实际是 21 天前的。
...
> 🔴 **组合收益率的两个口径（2026-09-05 澄清，实现时必须选对）**
>
> | 口径 | 公式 | 别名 | 09-04 值 | 09-05 复测 |
> |---|---|---|---|---|
> | **A（必须用于给用户看）** | `总市值 / 总成本 − 1` | 成本加权平均收益率 | −2.21% | **−3.99%** |
> | **B（禁止用于给用户看）** | `Σ(weight_mv_i × r_i)` | 市值加权平均收益率 | −0.02% | −2.05% |
>
> - **`B ≥ A` 恒成立**，差由 `B − A = Var_c(r)/(1+A)` 精确决定（`Var_c(r)` = 收益率按**成本**权重的加权方差）。2026-09-05 服务器实测：A = −3.9929%、B = −2.0479%、B−A = **+1.9450 pct**，恒等式预测 +1.9450 pct，**残差 ≈ 0**（残差仅来自 `portfolio.py:149` 把 `weight_mv` 取整到 2 位小数）。
> - 原因：`weight_mv_i ∝ cost_i ×(1+r_i)`，涨得多的基金自动拿到更大权重 → B 系统性高估收益。**用 B 会把「浮亏 3.99%」报成「浮亏 2.05%」，差 1.95pct。**
> - **`weight_mv` 在本代码库中只用于 xray 穿透二次加权 / render 展示 / 快照字段带出，没有任何地方用它加权收益率。** 全库算组合收益率处（`portfolio_overview.py:112-115`、`cfo_dashboard.py:224-225`、`api/portfolio.py:242-253`、`scripts/night_worker.py:947`、`dca.py:65`）**全部是口径 A**。
> - 本模板里的「组合整体」**一律按口径 A 计算**。原模板括号里写的「按市值加权」是**标注错误**，已改为「口径 A」。
>
> 详见 PRD §0.0.5。
...
> 🧪 **本任务原验收标准里有两个 `≈` 数值断言必须改写**（2026-09-04 修订）。原因：`61.8 / 24.4 / 13.8`、`33.5%`、`4.53%` 全部来自**等权测试持仓**（7×100 元 + 1×10 元），用户重录真实持仓后必然变化。若照原样写成断言，**重录当天 T03 的测试会全部变红，且变红是「正确行为」** —— 这会让测试失去信号价值，也让后来者误以为代码坏了。
>
> **改法：断言不变式，不断言数值。** 数值只写进注释作为「期望的量级」，不作为通过条件。

- `penetrated + blind + residual == 100.00`（±0.01）—— **保留为硬断言**（恒等式，与持仓数据无关，永远是这条）
  - ~~且 ≈ 61.8 / 24.4 / 13.8~~ 🧪 **降级为注释**：`# 等权测试持仓下的参考值：61.8 / 24.4 / 13.8（线上 63.0 / 25.1 / 12.0）`
  - 另补三条**与数值无关的结构断言**：`penetrated_pct >= 0`、`blind_pct >= 0`、`residual_pct >= 0`，三者均不得为负（防权重算错时出现负暴露）
- ~~半导体行业 exposure ≈ 33.5%；北方华创 ≈ 4.53%~~ 🧪 **改为**：
  - **行业聚合正确性**：手工用夹具数据算一遍某一个行业的 `exposure_pct`，与函数输出逐位比对（断言**算法**，不断言**数值**）
  - **个股聚合正确性**：同上，用夹具手算一只个股
  - ✅ `北方华创 fund_count == 4` —— **保留**（纯计数，与权重无关，重录后仍成立，除非用户真的调仓）
  - **二次加权恒等式**：`Σ_all_stocks exposure_pct == penetrated_pct`（±0.01），且 `penetrated_pct <= Σ weight_mv of ok funds`（穿透部分不可能超过可穿透基金的总权重）
- `blind_funds` 含 006555 / 005698，文案点名这两只 —— ✅ **保留**（由 `fund_portfolio` 返回 `ok=False` 决定，与权重无关）
- **覆盖率不依赖 `is_qdii`**：把所有基金的 `is_qdii` 强制置 False 重跑，**`blind_pct` 必须【与置 False 之前完全相等】**（§8.2 实测：`fund_type` 判不出 QDII，只能靠运行时 `ok=False`）
  - 🧪 原写法「`blind_pct` 必须仍是 24.4%」**已改写**：它把「不变式」和「等权测试值」捆在了一起。真正要守的是「**改 `is_qdii` 不影响 `blind_pct`**」这条不变式，而不是 `blind_pct` 等于多少。
...
> 🔴🧪 **夹具核查发现（2026-09-04）：现在存在三套互相矛盾的持仓数据，都不具备业务真实性。**
>
> | 来源 | 性质 | 说明 |
> |---|---|---|
> | 服务器 `/opt/moneybag/data/fund_holdings_LeiJiang.json` | **等权测试数据** | 7 只各 100 元 + 007356 十元；8 条 `addedAt` 全同；总成本 709 元 |
> | 本仓库 `backend/tests/fixtures/fund_holdings_leijiang.json` | **第三套编造数据** | 与服务器**不同构**：`shares` 是 12000/8000/16000…（服务器是 41.33/38.55/33.04…），且 **3 只基金的 `costNav` 与服务器对不上** |
> | 用户真实持仓 | **尚未录入** | 唯一可信来源 |
>
> **`costNav` 冲突明细**（同一只基金，两套数据给出不同的成本价）：
>
> | code | 夹具 `costNav` | 服务器 `costNav` | 差 |
> |---|---|---|---|
> | 016501 | **1.5000** | 3.0266 | -50.4% |
> | 006555 | **2.0000** | 3.4804 | -42.5% |
> | 007356 | **1.8000** | 4.9020 | -63.3% |
> | 013107 / 002163 / 005851 / 008984 / 005698 | 一致 | 一致 | — |
>
> **影响**：凡是用夹具跑 P0-3 回撤的测试（`test_fund_drawdown_state.py`），`dd_cost = unit_nav / cost_nav - 1` 里 `cost_nav` 取自夹具 → **这 3 只基金的回撤值会与生产环境严重不符**（例如 007356 成本价差 63%，档位判定会完全不同）。这不影响状态机逻辑本身的正确性（测的是跃迁规则，不是数值），但**会让「用夹具跑出来的档位」看起来像真的一样，而它不是**。
>
> **建议**（不阻塞本次，登记待办）：
> 1. 夹具**不要**声称「与服务器同构」—— 它不同构，注释需改；
> 2. 待用户重录真实持仓后，**用真实持仓（脱敏后）重做夹具**，让测试基线回归真实；
> 3. 在此之前，P0-3 的测试**只断言状态机跃迁与推送次数，不断言具体 `dd` 数值**。
