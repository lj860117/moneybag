...
from services.wxwork_push import (
    is_configured as wxwork_configured, send_text as wxwork_send,
    verify_callback as wxwork_verify, decrypt_message as wxwork_decrypt,
    encrypt_reply as wxwork_encrypt_reply, send_markdown,
)
...
async def wxwork_callback_receive(
    request: Request,
    msg_signature: str = "",
    timestamp: str = "",
    nonce: str = "",
    ...
    # 快速回复"已收到"（企微要求 5 秒内响应）
    # 然后异步调 DeepSeek，分析完后主动推送结果
    def async_reply():
        try:
            ...
            if cmd in ("持仓", "我的持仓", "持仓情况"):
                _handle_wxwork_holdings(from_user, user_id, user_name)
                return
            if cmd in ("扫描", "盯盘", "异动"):
                _handle_wxwork_scan(from_user, user_id, user_name)
                return
            if cmd in ("帮助", "help", "?", "？"):
                _handle_wxwork_help(from_user)
                return
            ...
            if not api_key:
                send_markdown(f"⚠️ AI 暂不可用（未配置 API Key）", user_id=from_user)
                return
...
def _handle_wxwork_holdings(wxwork_uid: str, user_id: str, user_name: str):
    """快捷指令：查看持仓"""
...
def _handle_wxwork_scan(wxwork_uid: str, user_id: str, user_name: str):
    """快捷指令：触发盯盘扫描"""
...
def _handle_wxwork_help(wxwork_uid: str):
    """快捷指令：帮助"""
    help_text = """**🤖 钱袋子 AI 助手**
...
# ---- 邀请码系统 ----
_ADMIN_KEY = "moneybag_admin_2026"  # 管理员密钥
