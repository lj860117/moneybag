...
每周日凌晨 2 点由 systemd timer 触发，对：
  ...
  3. 把采样摘要喂给 DeepSeek V4 Flash，让 LLM 找逻辑矛盾和异常
...
设计约束：
  ...
  - 每周只跑一次 → LLM 审计用 V4 Flash（2026-09-11 全面 Flash 化；内部调用一律便宜档）
...
def run_llm_audit(probe_results: list[dict[str, Any]], smoke_results: list[dict[str, Any]]) -> dict[str, Any]:
    """
    把数据摘要喂给 V4 Flash（2026-09-11 全面 Flash 化），让 LLM 找：
...
- 13维信号（多因子综合打分，输出 HOLD/BUY/SELL 仓位建议）和 Regime判断（基于均线排列+20日涨跌+波动率的技术面状态分类）是【两套相互独立的算法】，输入维度完全不同。因此"Regime=高波熊市（趋势状态）"与"13维信号=HOLD（综合建议）"同时出现【不是信号矛盾】：Regime 只描述"当前处于下跌+高波动趋势"，13维信号则是把估值/资金/情绪/宏观等13个维度综合后得出"多空分歧、暂不操作"的结论，两者方向不同是设计使然，不是打分 bug。只有当 13维信号 的 overall 明显与 score 数值矛盾（如 overall=BUY 但 score 大幅为负，或 overall=SELL 但 score 大幅为正）才需要上报。
- 各数据源的数据日期（data_date）天然存在 T+1/T+2 滞后差异：全球市场/行情类数据多为当日收盘后更新（T+0 或 T+1），SHIBOR/两融/国债收益率等结算/报价类数据通常滞后1-2个交易日。跨模块出现"全球市场=最新交易日、SHIBOR=前一交易日、融资融券=前两交易日"这类 1-3 天的时间基准差异【是数据源本身的更新节奏不同，不是缓存失效或数据停滞】。只有当某个数据源的 data_date 比最新交易日落后超过 3 个交易日（约一周以上）才需要上报为"数据可能失效"。
...
请从以下角度分析剩余的真实问题：
1. **数据逻辑一致性**：排除上面已知的正常不同步关系后，是否还存在其他明显矛盾？
2. **数据新鲜度/可信度**：哪些数据看起来可能已经失效或长期未更新？
3. **功能风险**：基于测试结果，哪些功能模块对用户的判断可能产生误导？
4. **综合健康评分**：给本次审计打0-100 分（100=完全健康），并说明扣分原因。
