...
# 添加项目根目录到 Python 路径。
# 必须 .resolve()：cron 是 `cd /opt/moneybag/backend && python scripts/migrate_phase3.py`，
# 此时 __file__ 是相对路径 `scripts/migrate_phase3.py`，不 resolve 的话
# parent.parent.parent 会得到 `.`（=backend/），于是 `import backend.services...` 炸。
project_root = Path(__file__).resolve().parent.parent.parent
if str(project_root) not in sys.path:
    sys.path.insert(0, str(project_root))

# 仅插仓库根不够：`from config import DATA_DIR` 和 backend.services.* 内部
# 的 `from infra...` 都要以 backend/ 为根解析。以 `python scripts/migrate_phase3.py`
