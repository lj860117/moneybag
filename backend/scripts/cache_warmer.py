...
def warm_midday():
    ...
    # 2026-09-19：预热节奏从「每 30min」降为「每 1h」（crontab 改 0 9-14），
    # TTL 相应从 1h 提到 2h —— 不是为了存更久，而是**留一轮容错余量**：
    #   旧：每 30min 刷 + TTL 1h → 缓存永远不过期，等于一半的刷新是纯冗余
    #       （刷新时上一份还剩 30min 有效期），LLM 调用白烧一倍。
    #   新：每 1h 刷 + TTL 2h → 万一某轮预热失败，缓存还能再撑 1 小时，
    #       不会立刻穿透成用户请求触发的实时 LLM 调用（那更贵也更慢）。
    # 盘中新闻/资金流的新鲜度由 30min 放宽到 60min，属可接受范围。
    ttl = 2
