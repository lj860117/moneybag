...
def _build_portfolio_thermometer_with_data(uid: str) -> tuple:
    ...
    try:
        ...
        for r in rows:
            # 不能裸 [:12]：会把 (QDII) 截成 (Q，留下未闭合括号
            # （详见 _shorten_fund_name 的注释）。名称被括号吃光时回落到代码。
            name_short = _shorten_fund_name(r["name"], 12) or r["code"]
            if r["navMissing"]:
...
def _inject_hallucination_label(briefings: dict) -> dict:
    ...
    # Bug4 修复（2026-09-09）：原正则以 `\s*$` 锚定，要求限定语与百分比
    # **紧邻**才生效。而晨报的标准写法是「本基金近3年涨幅320%」——限定语和
    # 数字之间夹了「涨幅 / 回报 / 上涨」这类中文词，`$` 锚定即失效，导致
    # 合法的长周期涨幅被误标成幻觉数字（9-8 凌晨 10 条「异常涨幅数字320%」）。
    # 去掉 `\s*$` 改为非锚定搜索：限定语只要落在数字前 12 字回看窗口内即
    # 视为合规表述。
    #
    # 去锚定的副作用（同日发现并修）：时长单位表里原有 `个月|月|周|日|天`，
    # 这些根本不是长周期 —— A 股一天涨 320% 是不可能的，属典型幻觉数字。
    # 旧代码下「近1天 320%」（紧邻写法）本就被豁免，去锚定后连
    # 「近1天涨幅 320%」这类非紧邻写法也一起豁免了，等于把原有的洞放大。
    # 故把单位表收窄到只留 `年`：短周期（天/周/月）一律不再豁免。
