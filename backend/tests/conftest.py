...
#
# ---------------------------------------------------------------------------
# 关于最后一条 `backend/tests` 的裁决（2026-09-18 定，勿重复推翻）
# ---------------------------------------------------------------------------
# 它曾被认真考虑摘掉，理由是"多人并行开发时必误报"：A 同学跑测试的会话期间，
# B 同学新增/改写了一个测试文件 → A 的快照对比把 `backend/tests` 判成 changed
# → 开火 → A 得到一个**与他毫无关系**的红。这个误报是真的，实测发生过。
#
# **裁决：保留。** 理由是比较两种失效模式：
#   - 保留 → 代价是**并行时误报**（噪音红）。噪音红虽然烦，但它指向的是一个
#     真实存在的事实（那棵树确实变了），只是"责任归属"错了；排查时一看清单
#     里的路径是不是自己动的，30 秒即可消化。
#   - 摘掉 → 代价是**再也抓不到「测试写测试目录」**。本仓历史上确实有测试把
#     产物写进 `backend/tests/`（那正是这条树被加进来的原因）。这类问题一旦
#     失去守卫就是**静默**的：仓库里慢慢长出一堆没人认领的文件，谁也不知道
#     是哪个测试干的。
#   误报 vs 静默漏报 —— 按本项目一贯的取向（宁可噪音红，不要静默绿）选保留。
#
# **规避纪律（这才是正确的解法）**：误报的根因是**并行**，不是这个设计。
#   所以约束的是"怎么跑测试"，不是"守哪些树"：
#     - 跑**全量**时，同一工作区里不要有别人同时在写文件（含改代码、新增测试、
#       跑另一个 pytest 会话）。并发 pytest 会话会互相污染快照基线，反而制造
#       更难查的假红/假凶手。
#     - 需要并行干活时，让写代码的人各自用独立副本（`git clone` 到 /tmp）跑测试，
#       或把改动做完再统一跑一次全量。
#     - 万一守卫开火：**先看 stderr 清单里的具体路径**判断是谁动的，不要因为
#       "清单里有别人的文件"就以为守卫坏了，更不要去删清单里的文件。
#
