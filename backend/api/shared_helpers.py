...
#
# 目标名 = 一段「不含空白 / 疑问词 / 标点」的连续字符，遇到 `吗/么/嘛/？/。/,` 立即截断。
# **刻意不用 `$` 锚定行尾**：这三类问句在实际语料里经常内嵌在长句中，例如本次线上
# 观测到的原始句 `我持有宁德时代吗？没有就说没有。`——锚定行尾时它抽不出标的，
# 整句落到兜底话术；去掉句号后又会因非贪婪 + `$` 回溯出 `宁德时代吗？没有就说`
# 这种半句垃圾并原样回显给用户。用否定字符类截断，两种毛病一次解决。
# （`我持有\s*` 里的 `\s*` 是必要的：`我持有 贵州茅台 吗` 这种带空格的也要抽得出。）
_TARGET_CHARS = r'[^\s吗么嘛？?。，,！!；;：:（(）)]+'
_SPECIFIC_HOLDING_PATTERNS = [
    _re.compile(r'我持有\s*(' + _TARGET_CHARS + r')'),
    _re.compile(r'我有没有\s*(' + _TARGET_CHARS + r')'),
    _re.compile(r'是不是持有\s*(' + _TARGET_CHARS + r')'),
]

# ⚠️ 已知边界（有意保留，勿"顺手删掉「股票」"）：这里把「股票/基金」当分类词拦住，
# 因此「我持有贵州茅台的股票吗」会抽不出标的、fall through。
# 这是可接受的：规则引擎只是快路径，AI 正常时由 LLM 结合 portfolio_ctx 正确回答；
# 仅 AI 降级时落到「AI 暂时不可用」——那是真话，不是假理由。
# 想真正支持它需要给记录模型加"品种"语义，但 portfolio_ctx 两种格式里
# 格式①（`- 贵州茅台(600519)：¥80,000`）**不带品种**，原理上无法判定，必须走
# 「无法确定 + 原因=记录不区分品种」。属独立一轮的设计变更，故此处只记录、不改。
...
def _rule_based_reply_structured(msg: str, market_ctx: str, portfolio_ctx: str) -> dict | None:
    ...
    _HOLDING_QUERY_KW = ["我有什么", "我的持仓", "我的资产",
                          ...
                          "我买了什么", "我买了哪些"]
    # 含有决策意图的【持仓问题】→ 必须交给 LLM 做分析（规则引擎只能列持仓，不能给建议）
    #
    # ⚠️ 2026-09-13 修复（rules-probe 发现）：限定词「持仓」此前**只写在注释里，没有实现**。
    # 旧代码判到 `_has_decision_intent` 就直接 return None，于是任何含决策词的消息都被
    # 拦到 LLM —— 包括「止盈」「定投多少合适」这类**通用咨询**。后果是下面 :1321 的
    # take_profit 规则和 :1326 的 dca 规则**成了死代码**：用户问「止盈」既拿不到那张
    # 现成的止盈止损表，还要多付一次 LLM 调用（毫秒 → 秒级）。
    # 该缺陷 2026-05-24 (`80b67cd`) 引入，长期被「本地根 tests 全部 skip（后端未启动）」
    # 掩盖，4 个月未被任何测试发现。
    # 现在按注释本意补上限定：只有**引用了自己持仓**的决策问题才交给 LLM。
    _DECISION_KW = ["定投", "卖出", "卖掉", "减仓", "加仓", "止盈", "止损",
                    "继续", "要不要", "该不该", "值得", "怎么办", "如何操作",
                    "换仓", "调仓", "赎回", "申购", "对不对", "合不合适"]
    # 自我指涉信号：消息里提到"我的持仓 / 我持有 / 我买的…"才算持仓决策，需要个性化分析。
    # ⚠️ 不能复用 _HOLDING_QUERY_KW —— 它（在下方定义）偏向"列出全部持仓"的完整句式
    # （"我有什么""我的持仓"…），而"我持有的基金要止盈吗"并不匹配它。这里要的是更宽的
    # "是否提到自己"的信号。
    _SELF_HOLDING_KW = ["我的", "我持有", "我买", "我有", "我现在", "我这", "我还", "自己"]
    _has_decision_intent = any(k in msg_lower for k in _DECISION_KW)
    _is_holding_decision = _has_decision_intent and any(k in msg_lower for k in _SELF_HOLDING_KW)
    if _is_holding_decision:
        return None  # 持仓相关的决策意图，必须走 LLM 做个性化分析
