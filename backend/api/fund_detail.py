...
def fund_nav_history(code: str, days: int = 90, name: str = ""):
    ...
    is_qdii（v9.9.38）：
      ...
      **境内人民币**基金会被判成 USD。
      ...
      ⚠️ 措辞更正（2026-09-16 生产实测）：「用户一点切换就把整条 K 线 ÷7.2，
      是数值错误」这个说法在**当前线上不可达**，别再照抄。实测依据：本接口
      is_qdii 判定准确（探针 501310→false、006555→true），前端撤除逻辑会执行；
      且 fetch 失败路径无数据可除。当前可观察后果是非正常路径下切换条残留、
      点了没反应 —— 错误可操作性，属 UX 缺陷。仅当本接口漂移/未回传 is_qdii
      时才退化为数值错误。v9.9.44 前端已改为「确认 is_qdii===true 才插入」
      的失效安全写法。
    """
