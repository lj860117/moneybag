...
async def _aiter_blocking(gen: Iterator[Any]) -> AsyncIterator[Any]:
    ...
    取舍（已知代价，勿盲目照搬）：
    ① 每次取一个 chunk 会占用一个线程池线程。默认池大小是
       ``min(32, cpu_count + 4)`` —— **生产机 nproc=2，池上限只有 6**
       （2026-09-18 独立复验实测核实；本开发机 18 核时为 22）。
       单进程并发流式路数超过该值即排队：本机 25 路并发时最慢一路耗时
       2.77s vs 理论 1.80s，多出的 ≈0.97s 恰约 3 个 chunk 的串行代价。
       两个用户的规模够用，但**别照搬"30 路"这个数字** —— 它只在多核
       开发机上成立，生产是 6。
