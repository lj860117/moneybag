...
def _find_cut(text: str, cum: list, start: int, limit_bytes: int, budget: int) -> int:
    ...
    两条优先级，**条数优先于行边界**：
      ① 硬约束（只约束「候选切点」）—— 任何会让总条数超过理论最少 k 的候选
         （含「整齐的板块标记」和「行边界换行」）一律放弃。
      ② 软偏好 —— 在①的前提下，尽量切在「板块标记」或「行边界」，避免把一段
         或一行从中间劈开。
    ①与②冲突时（预算内的最后一个换行 < floor_bytes，即用它必然多出一条）
    退化为硬切在预算边界 —— 此时**一行会被劈开**，这是刻意的取舍，非 bug。

    ⚠️ ① 有一个**无法避免的例外**（2026-09-18 独立验证实测；此缺陷在本次改动
    之前就存在，非本次引入，且新实现**从不比旧实现差**）：
    最后的兜底 `return hi` 不受①约束。当预算边界恰好落在**多字节字符中间**时，
    `hi` 只能装下 `budget-(w-1)` 字节（w = 该字符的 UTF-8 宽度），于是剩余会
    > `(k-1)*budget` → **总条数比理论最少多 1**。
    最小反例：`"a"*1799 + "中" + "a"*1798`（恰好 3600 字节）按 1800 预算 →
    实际 3 条 `[1799, 1800, 1]`，而理论最少是 2 条。
    中英混排+emoji 随机 3000 例中约 **2.6%** 触发（旧实现约 **69.6%**）。
    这是「字节预算」与「字符边界」的固有冲突，除非允许某一段超预算，否则无解 ——
    所以**不要把它当"必须修"的 bug**。真实晨报每行都有换行、切点落在换行处，不触发。
    """
    ...
    #
    # 教训（2026-09-17 LeiJiang 晨报**生产入参** 3532B = 52B 信封 + 压缩后正文 3480B）：
    # 本该 ceil(3532/1800)=2 条，却因标记优先切在 1287B（「持仓明细」处），
    # 硬生生变成 3 条 [1287, 1577, 668]，切点还把「持仓明细」的一行基金数据劈成两半。
    # 注意口径：存档正文是 3580B（不含信封、未压缩），不是生产入参 —— 见 qa 复核。
    #
