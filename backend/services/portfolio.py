...
# 风险等级 → 基础配置模板
# v9.9.26 P1-9: 修正枚举错位。此前这里的键是「积极型」，而系统权威枚举
# （config.RISK_ALLOC_PCTS、问卷 getProfile）用的是「进取型」——于是**进取型
# 用户在 _allocate_picks 里查不到模板，静默回退到稳健型配置**（股 50% 而非
# 70%），与用户风险偏好相反，且没有任何日志。
# 现在以权威枚举「进取型」为准；「积极型」保留为别名，避免历史调用方静默回退。
# 防复发见 tests/test_ai_fund_no_fabrication.py 的枚举一致性断言。
RISK_TEMPLATES = {
    "保守型": {"stock": 0.20, "bond": 0.60, "cash": 0.20},
    "稳健型": {"stock": 0.50, "bond": 0.30, "cash": 0.20},
    "平衡型": {"stock": 0.50, "bond": 0.30, "cash": 0.20},
    "进取型": {"stock": 0.70, "bond": 0.15, "cash": 0.15},
    "积极型": {"stock": 0.70, "bond": 0.15, "cash": 0.15},  # 别名：等同进取型
