...
<button class="action-btn primary" onclick="showAllocateAssets()">💰 配置资产</button>
...
async function sendChat(text){if(_chatSending)return;const inp=document.getElementById('chatIn');const msg=text||(inp?inp.value.trim():'');if(!msg)return;_chatSending=true;if(inp)inp.value='';
...
chatMessages.push({role:'bot',text:fullText,src:source});_chatSending=false;return}
// 非流式降级（旧接口兼容）
if(r.ok){const d=await r.json();chatMessages.push({role:'bot',text:d.reply,src:d.source});appendMsg('bot',d.reply,d.source);_chatSending=false;return}
}catch(e){console.warn('Chat stream error:',e)}}
rmTyping();const fb='后端未连接，无法获取实时数据。请确保后端运行中。';chatMessages.push({role:'bot',text:fb,src:'offline'});appendMsg('bot',fb,'offline');_chatSending=false}
