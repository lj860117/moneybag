...
.news-item{padding:12px 0;border-bottom:1px solid var(--bg3);display:flex;gap:10px;align-items:flex-start}.news-item:last-child{border-bottom:none}.news-icon{font-size:18px;flex-shrink:0;margin-top:2px}.news-content{flex:1}.news-title{font-size:14px;line-height:1.5}.news-meta{font-size:11px;color:var(--text2);margin-top:4px}
.macro-item{display:flex;gap:12px;padding:12px 0;border-bottom:1px solid var(--bg3);align-items:flex-start}.macro-item:last-child{border-bottom:none}.macro-icon{font-size:24px;flex-shrink:0}.macro-info{flex:1}.macro-name{font-size:14px;font-weight:600}.macro-value{font-size:18px;font-weight:900;color:var(--accent);margin:4px 0}.macro-impact{font-size:12px;color:var(--text2);line-height:1.4}.macro-date{font-size:11px;color:var(--text2)}
...
const dash=await fetchDashboard();if(!dash){document.getElementById('insightContent').innerHTML='<div style="text-align:center;padding:40px;color:var(--text2)">数据加载失败</div>';return}
...
el.innerHTML=`
<div class="dashboard-card"><div class="dashboard-card-title">😱 恐惧贪婪指数</div>
...
<div class="dashboard-card"><div class="dashboard-card-title">📊 估值水平</div>
...
<div class="dashboard-card"><div class="dashboard-card-title">📈 技术指标</div>
<div class="tech-grid">
...
${news.length?`<div class="dashboard-card"><div class="dashboard-card-title">📰 最新资讯</div>${news.map(n=>`<div class="news-item"><div class="news-icon">📰</div><div class="news-content"><div class="news-title">${n.title}</div><div class="news-meta">${n.source||''}${n.time?' · '+n.time:''}</div></div></div>`).join('')}</div>`:''}
${macro.length?`<div class="dashboard-card"><div class="dashboard-card-title">🏛️ 宏观经济</div>${macro.map(e=>`<div class="macro-item"><div class="macro-icon">${e.icon||'📅'}</div><div class="macro-info"><div class="macro-name">${e.name}</div><div class="macro-value">${e.value||'—'}</div><div class="macro-impact">${e.impact||''}</div></div></div>`).join('')}</div>`:''}
...
const news=d.news||[];
el.innerHTML=`<div class="dashboard-card"><div class="dashboard-card-title">📰 市场新闻</div>${news.length?news.map(n=>`<div class="news-item"><div class="news-icon">📰</div><div class="news-content"><div class="news-title">${n.title}</div><div class="news-meta">${n.source||''}${n.time?' · '+n.time:''}</div></div></div>`).join(''):'<div style="text-align:center;padding:20px;color:var(--text2)">暂无新闻</div>'}</div>
...
el.innerHTML=`
<div class="dashboard-card"><div class="dashboard-card-title">📊 RSI 相对强弱指标</div>
...
<div class="dashboard-card"><div class="dashboard-card-title">📈 MACD 指数平滑移动平均</div>
...
<div class="dashboard-card"><div class="dashboard-card-title">📐 布林带</div>
...
<div style="text-align:center;padding:16px;font-size:12px;color:#475569;line-height:1.6">💡 技术指标是辅助参考，需结合估值和基本面综合判断。<br>RSI>70可能超买，<30可能超卖；MACD金叉看多、死叉看空。</div>`}

function renderInsightMacro(el,d){
const macro=d.macro||[];
el.innerHTML=`<div class="dashboard-card"><div class="dashboard-card-title">🏛️ 宏观经济数据</div>
${macro.length?macro.map(e=>`<div class="macro-item"><div class="macro-icon">${e.icon||'📅'}</div><div class="macro-info"><div class="macro-name">${e.name}</div><div class="macro-value">${e.value||'—'}</div><div class="macro-date">${e.date||''}</div><div class="macro-impact">${e.impact||''}</div></div></div>`).join(''):'<div style="text-align:center;padding:20px;color:var(--text2)">暂无数据</div>'}
</div>
<div style="padding:16px;font-size:12px;color:#475569;line-height:1.6">💡 宏观经济数据影响市场整体方向：<br>• CPI 上升 → 通胀压力 → 可能加息 → 股市承压<br>• PMI > 50 → 经济扩张 → 利好股市<br>• M2 增速加快 → 流动性宽松 → 利好资产价格</div>`}
